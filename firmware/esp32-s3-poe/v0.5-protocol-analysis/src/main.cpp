...
void printRouteBytes(uint8_t src, uint8_t dst, uint8_t type) {
  Serial.printf("%02X->%02X type=%02X", src, dst, type);
}

void printRouteForFrame(const uint8_t *buffer, size_t length) {
  if (length >= 4) {
    printRouteBytes(buffer[1], buffer[2], buffer[3]);
  } else {
    Serial.print("--");
  }
}

void printIndexMask(uint32_t mask, bool beyondMask, bool varyingLength) {
...
void printFrameFamilySummaries() {
  for (size_t index = 0; index < kFrameFamilyCapacity; ++index) {
    ...
    Serial.printf("[family] id=%u route=", static_cast<unsigned>(index + 1));
    printRouteBytes(family.routeSrc, family.routeDst, family.routeType);
    Serial.print(" sig=");
    printSignature(family.signature, family.signatureLength);
    Serial.printf(
        " seen=%u last_len=%u last_gap_us=%u avg_gap_us=%u vary_idx=",
        family.seenCount,
        static_cast<unsigned>(family.lastFrameLength),
        family.lastGapUs,
        avgGapUs);
    printIndexMask(family.varyingMask, family.varyingBeyondMask, family.varyingLength);
    Serial.println();
  }
}
...
void flushFrame(const char *reason) {
  ...
  Serial.print(" route=");
  printRouteForFrame(g_frameBuffer, g_frameLength);
  Serial.printf(
  ...
  Serial.print(" changed_idx=");
  printIndexMask(lastChangeMask, false, lastLengthChanged);
  Serial.print(" data=");
