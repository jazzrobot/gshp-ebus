...
This is the third versioned listener iteration after successful live eBUS capture on the Waveshare `ESP32-S3-ETH`.

It builds on `v0.3` by switching to quieter frame-first logging during early protocol analysis.
...
- `v0.4` is an analysis-focused iteration that should be promoted to `current/` only after its log output proves more useful than `v0.3` on the live system.
