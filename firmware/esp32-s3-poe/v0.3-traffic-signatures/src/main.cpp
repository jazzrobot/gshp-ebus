...
constexpr size_t kSignatureBytes = 5;
...
struct ListenerStats {
  ...
  uint32_t suppressedSyncByteLogs = 0;
...
void handleIncomingBytes(size_t byteCount) {
  ...
  while (byteCount > 0) {
    ...
    for (int index = 0; index < readCount; ++index) {
      ...
      if (suppressSyncByteLog) {
        ++g_stats.suppressedSyncByteLogs;
      } else {
        Serial.printf(
...
void printSummary() {
  ...
  Serial.printf(
      "[stats] up_ms=%lu bytes=%llu frames=%llu families=%u sync_only=%u sync_rx_supp=%u trunc=%u frame_err=%u parity_err=%u fifo_ovf=%u buffer_full=%u breaks=%u unknown=%u\n",
